...
@testset "boundary_decomposition" begin
    A = rand(5, 2)
...
@testset "exact decomposition" begin
    A = rand(2, 3, 2)
    T = left_right_kron(A)
...
@testset "multiple sites" begin
    ...
    @test replicated_expectation([A1, A2, A3]) ≈ dot(T1, T2 * T3)
...
@testset "truncated rank" begin
    ...
    @test replicated_expectation([A1, A2, A3, A4]; ranks) ≈ replicated_expectation([A1, A2, A3, A4]) rtol=1e-1        
end
